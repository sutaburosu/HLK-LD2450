...
uint8_t LD2450::read()
{
  ...
  int avail = LD2450::radar_uart->available();
  if (avail < 30)
    return -1;

  byte rec_buf[LD2450_SERIAL_BUFFER] = "";
  const int len = LD2450::radar_uart->readBytes(rec_buf, min(avail, (int)sizeof(rec_buf)));
  if (len)
    ...
    if (rec_buf[i] == 0xAA && rec_buf[i + 1] == 0xFF && rec_buf[i + 2] == 0x03 && rec_buf[i + 3] == 0x00 && rec_buf[i + 28] == 0x55 && rec_buf[i + 29] == 0xCC)
    {
      ...
      for (uint16_t targetCounter = 0; targetCounter < LD2450_MAX_SENSOR_TARGETS; targetCounter++)
      {
        ...
        if (index + 7 < len)
        {
          ...
          // CALCULATE DISTANCE
          target.distance = sqrtf(powf(target.x, 2) + powf(target.y, 2));
          ...
          if (target.x < LD2450::radarTargets[targetCounter].xmin)
            LD2450::radarTargets[targetCounter].xmin = target.x;
          if (target.x > LD2450::radarTargets[targetCounter].xmax)
            LD2450::radarTargets[targetCounter].xmax = target.x;
          if (target.y < LD2450::radarTargets[targetCounter].ymin)
            LD2450::radarTargets[targetCounter].ymin = target.y;
          if (target.y > LD2450::radarTargets[targetCounter].ymax)
            LD2450::radarTargets[targetCounter].ymax = target.y;
